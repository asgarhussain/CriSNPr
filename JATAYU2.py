...
def connect_db():
    sql=sqlite3.connect('/home/asgar/flask/crispsnps/db/ngr_nrg_snp_wd_freq.db')
...
def offtargets():
        ...
        file1 = open(f1,"w")
        if my_var2 == 'Human':
            ref='/home/asgar/flask/jatayu/GRCh38.p13.genome.fa'
        else:
            ref='sequence.fasta'    
...
def align(wtype,len1,position,variation,ref,organism):
  ...
  if organism =='Homo Sapiens':
    cmdArgs=['bwa' ,'mem'  ,'-t' ,'8', '-k', str(len1) , '-T' , '20', '-a' ,'/home/asgar/flask/jatayu/GRCh38.p13.genome.fa',filename, '-o' ,filename_sam] # 'bwa', 'mem' ,'a','-t', '4'  ,'GRCh38.p13.genome.fa' , 'example.fasta',  '-o', 'example.sam'  
    call(cmdArgs)
    ...
    if df['CHROMOSOME'][0] == '*':
     ...
     if results:
            ...
            cmdArgs3=['bedtools', 'intersect', '-a', str(filename_bed),'-b', '/home/asgar/flask/jatayu/gencode.v32.annotation_genes_filtered.gtf' , '-wb'   ]
    ...
    cmdArgs=['bwa' ,'mem'  ,'-t' ,'8', '-k', str(len1) , '-T' , '20', '-a' ,'/home/asgar/flask/crispsnps/sequence.fasta',filename, '-o' ,filename_sam] # 'bwa', 'mem' ,'a','-t', '4'  ,'GRCh38.p13.genome.fa' , 'example.fasta',  '-o', 'example.sam'  
    call(cmdArgs)
    df=pd.read_csv(filename_sam,sep='\t',header=None,comment="@")
    df=df.iloc[:,0:12]
    df.columns=['QNAME','Strand','CHROMOSOME','Start','MAPQ','CIGAR','RNEXT','PNEXT','TLEN','SEQ','QUAL','NM']
    ...
    if df['CHROMOSOME'][0] == '*':
     ...
     if results:
            ...
            cmdArgs3=['bedtools', 'intersect', '-a', str(filename_bed),'-b', '/home/asgar/flask/crispsnps/sars_cov2_filtered.gtf' , '-wb'   ]
            call(cmdArgs3,stdout=open(filename_bed_gene,'w'))
            bedgene=pd.read_csv(filename_bed_gene,header=None,sep="\t")
            bedgene=bedgene.iloc[:,[0,4, 5,6,10]]
